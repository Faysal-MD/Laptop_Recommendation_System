...
CREATE OR REPLACE PACKAGE Body predict_laptop_site2 AS
	...
	BEGIN
		For R in (select pid,absolute_value(memory, A) as df_memory, absolute_value(storage, B) as df_storage,
			...
			absolute_value(display, E) as df_display from specification2@site2) LOOP
						 
			insert into difference_table2@site2 values(R.pid, R.df_memory, R.df_storage, R.df_cpu_speed, R.df_battery, 
			R.df_display);
		End Loop;		
